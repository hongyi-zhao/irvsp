      SUBROUTINE CHRCT(NMAT,NUME,A,B,SU2,EE,LMAT,XM,FL,NE,NV,LKG,IKG,IORD,KKK)
...
!     COMPLEX*16       PH(NSYM,NMAT),XM(NSYM,NUME)
      COMPLEX*16       XM(NSYM,NUME)
      COMPLEX*16       A(NMAT,NUME),B(NMAT,NUME),LMAT(NMAT,NMAT,NSYM)
...
!                   SU2(2,2,LKG(IG))*B(I,J))*PH(IG,I)
         ...
         A2(:,J2)= SU2(1,1,LKG(IG))*MATMUL(LMAT(:,:,LKG(IG)),A(:,J)) &
                 + SU2(1,2,LKG(IG))*MATMUL(LMAT(:,:,LKG(IG)),B(:,J))
         B2(:,J2)= SU2(2,1,LKG(IG))*MATMUL(LMAT(:,:,LKG(IG)),A(:,J)) &
                 + SU2(2,2,LKG(IG))*MATMUL(LMAT(:,:,LKG(IG)),B(:,J))
 350    CONTINUE
...
!                   +DCONJG(B(L(IG,I),IE+NP))*B2(I,1+NO)
 380       CSUM=CSUM+DCONJG(A(I,IE+NP))*A2(I,1+NO) &
                    +DCONJG(B(I,IE+NP))*B2(I,1+NO)
        GAM(NP+1,NO+1)=CSUM
