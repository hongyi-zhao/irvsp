...
! purpose : read eigenstates from WAVECAR
! status  : robust
...
!======================================================================
subroutine kptin(KKK,nmax)
implicit real*8 (a-h, o-z)
integer,intent(in)::KKK,nmax
...
!     SUB  KSYM(KKK,WK,NMAT,NV  ,KV,   A  ,   B  ,NUME, NE ,EE,L,PH,XM)
     !if(nmax>0 .and. nmax<=NUME) then
      if(nmax<=NUME) then !wzj 2019.12.30
      CALL KSYM(KKK,WK,NMAT,ncnt,KV,coeffa,coeffb,NUME,nmax,EE,L,PH,XM)
      else
      WRITE(*,*) "WARNING: nmax is larger than NUME"
