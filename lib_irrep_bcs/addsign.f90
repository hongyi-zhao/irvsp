    subroutine getsign_littg(num_doub_sym, rot_bilbao, SU2_bilbao, &
        ...
        integer,     intent(in)    :: num_doub_sym
        integer,     intent(in)    :: rot_bilbao(3,3,num_doub_sym)
        complex(dp), intent(in)    :: SU2_bilbao(2,2,num_doub_sym)

        integer,     intent(in)    :: num_littg_bilbao
        integer,     intent(in)    :: littg_bilbao(num_doub_sym)

        integer,     intent(in)    :: num_sym 
        integer,     intent(in)    :: rot_reorder(3,3,num_sym)
        complex(dp), intent(inout) :: SU2_reorder_getsignk(2,2,num_sym)

        integer,     intent(in)    :: num_littg_input
        integer,     intent(in)    :: littg_input(num_sym)
        
        integer,     intent(in)    :: kopeconjg_bb2inp(num_sym)

        integer     :: rot_littg_bilbao(3,3,num_littg_bilbao)
        complex(dp) :: SU2_littg_bilbao(2,2,num_littg_bilbao)

        integer     :: rot_littg_input(3,3,num_littg_bilbao)
        complex(dp) :: SU2_littg_input(2,2,num_littg_bilbao) 

        integer     :: i, j, l

        do i = 1, num_littg_bilbao 
            rot_littg_bilbao(:,:,i) = rot_bilbao(:,:,littg_bilbao(i))
            SU2_littg_bilbao(:,:,i) = SU2_bilbao(:,:,littg_bilbao(i))
        enddo 
        do i = 1, num_littg_input
            do j = 1, num_sym
                if (kopeconjg_bb2inp(j) == littg_bilbao(i)) exit 
            enddo 
            if (j == num_sym+1) stop "kopeconjg error"
            rot_littg_input(:,:,i) = rot_reorder(:,:,j)
            SU2_littg_input(:,:,i) = SU2_reorder_getsignk(:,:,j)
        enddo 

        call getsign(num_littg_bilbao, &
                     rot_littg_bilbao, SU2_littg_bilbao, &
                     rot_littg_input, SU2_littg_input)

        do i = 1, num_littg_input
            do j = 1, num_sym
                if (kopeconjg_bb2inp(j) == littg_bilbao(i)) then
                    exit
                endif 
            enddo 
            if (j == num_sym+1) stop "kopeconjg error"
            SU2_reorder_getsignk(:,:,j) = SU2_littg_input(:,:,i)
        enddo 

    end subroutine getsign_littg 
