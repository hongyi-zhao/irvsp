...
! project : wave_data
! history : 07/18/2014
! authors : Zhijun Wang  ( zjwang@mail.com )
! purpose : input from WAVECAR
! status  : robust
...
contains
      ...
      subroutine setarray()
       allocate (EE(NUME))         ; EE=0._dp
       allocate (igall(3,NMAT))    ; igall=0
       allocate (KV(3,NMAT))       ; KV=0._dp
       allocate (L(NSYM,NMAT))     ; L=0
       allocate (PH(NSYM,NMAT))    ; PH=0._dp
       allocate (coeffa(NMAT,NUME)); coeffa=0._dp
       allocate (coeffb(NMAT,NUME)); coeffb=0._dp
       allocate (XM(NSYM,NUME))    ; XM=0._dp
      end subroutine
...
!           warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE
!======================================================================
subroutine kptin(KKK,nele,num_k_skip)
implicit real*8 (a-h, o-z)
integer,intent(in)::KKK,nele,num_k_skip
...
!-----open FILE--------
      ...
      if ( NSPIN .ne. ispin) then
         write(0,*)   '*** error - OUTCAR is NOT consistent with WAVECAR on ISPIN' ! zjwang 10.16.2019
        !write(0,*)   '*** error - selected k=',NSPIN,' > max k=',ispin
         stop "Please re-do the band calculation!!!"
      endif
      if ( NKPTS .ne. nwk) then
         write(0,*)   '*** error - OUTCAR is NOT consistent with WAVECAR on NKPTS' ! zjwang 10.16.2019
        !write(0,*)   '*** error - selected k=',NKPTS,' > max k=',nwk
         stop "Please re-do the band calculation!!!"
      endif
      if ( NUME .ne. nband ) then
         write(0,*)   '*** error - OUTCAR is NOT consistent with WAVECAR on NBANDS' ! zjwang 10.16.2019
...
!---------tmp for 614
      if(KKK==num_k_skip+1) then
           write(614,'(I3,A9)') NSPIN*NKPTS,' : k-list'
           write(624,'(I3)') NSPIN*NKPTS-num_k_skip
         do iK=num_k_skip+1,NSPIN*NKPTS
            irec=3+(iK-1)*(nband+1)
      read(unit=10,rec=irec) xnplane,(wk(i),i=1,3), &
           (cener(iband),occ(iband),iband=1,nband)
            write(614,'(2X,I3,2X,3F12.8)') iK,(wk(i),i=1,3)
            write(624,'(3F12.6)') (wk(i),i=1,3)
         enddo
     write(614,'(I3,A30)') nele,' : the number of valence bands'

     write(614,*)
     write(614,"(A40)") '################---output---############'
      endif
!---------tmp for 614
...
!     SUB  KSYM(KKK,WK,NMAT,NV  ,KV,   A  ,   B  ,NUME, NE ,EE,L,PH,XM)
      CALL KSYM(KKK,WK,NMAT,ncnt,KV,coeffa,coeffb,NUME,NUME,EE,L,PH,XM,nele)
!
      deallocate(coeff)
      deallocate(cener)
      deallocate(occ)
