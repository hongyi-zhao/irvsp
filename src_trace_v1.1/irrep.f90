...
!
      ...
      do while(.true.)
         ...
         if(arg(1:lens) == '-skip' ) then
            iarg = iarg + 1;call get_command_argument(iarg,arg,lens,stat)
            read(arg,*) num_k_skip
            write(*,"('The first N k-points will be skipped for hse: ',I5)") num_k_skip
         endif
         if(arg(1:lens) == '-noe' ) then
            iarg = iarg + 1;call get_command_argument(iarg,arg,lens,stat)
            read(arg,*) nele
...
!-------check files: OUTCAR and WAVECAR!!!
!
      IF(nele==0.and.num_k_skip==0) THEN
       WRITE(0,*) '### to give a desirable number of occupied bands: M'
       WRITE(0,*) '### and skip the first N k-points for HSE by the following command:'
       WRITE(0,*) '###$ vasp2trace -skip N -noe M'
      ELSE
      IF(num_k_skip==0) THEN
       WRITE(0,*) '### to skip the first N k-points for HSE by the following command:'
       WRITE(0,*) '###$ vasp2trace -skip N'
      ENDIF
      IF(nele==0) THEN
       nele=nint(rele)
       IF(.NOT.FL2) THEN
...
!
!.....loop over k-points
      DO KKK=num_k_skip+1,NSPIN*NKPTS
      CALL KPTIN(KKK,nele,num_k_skip)
      ENDDO
!
      call downarray()
!
